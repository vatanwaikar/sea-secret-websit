...
const outletData: Record<OutletKey, { label: string; email: string; whatsapp: string }> = {
  deccan: {
...
export function ReservationForm({ compact = false }: ReservationFormProps) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const response = await fetch('/api/reservations', {
      ...
      body: JSON.stringify({ name, phone, email, date, guests, outlet, notes }),
    })
    ...
    if (!response.ok) {
      ...
      setError(data?.error ?? 'Unable to send your reservation request. Please try again later.')
      return
    }
  ...
  if (submitted)
    return (
      <div className="success-message">
        ...
        <p>Your request is with our team. We&apos;ll call shortly to confirm your table.</p>
        <button className="button button-navy" onClick={() => setSubmitted(false)}>
    ...
    <form onSubmit={submit}>
      ...
      {!compact && (
        <label>
          ...
          <textarea name="notes" rows={3} placeholder="A birthday, dietary note, or special request" />
        </label>
      )}
      <label>
        Choose your outlet
      ...
      {error ? <p style={{ color: 'red', marginTop: '0.85rem' }}>{error}</p> : null}
    </form>
  )
}
...
export function WhatsAppButton({
  ...
  const message = encodeURIComponent(`Hello SeaSecret, I would like to reserve a table at the ${outletInfo.label}.`)
